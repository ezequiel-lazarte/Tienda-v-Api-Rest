...
module.exports.SalesController = {
    getSales: async(req, res) => {
        try {
            ...
            Response.success(res, 200, 'Lista de usuarios', sales);
        } catch (error) {
            debug(error);
            Response.error(res);
        }
    ...
    updateSale: async(req, res) => {
        try {
            ...
            if(!sale) {
                ...
                Response.success(res, 200, `Venta ${id} modificada`, Object(body));
            }
        } catch (error) {
            debug(error);
            Response.error(res);
    ...
    deleteSale: async (req, res) => {
        try {
            const {params: {id}} = req;
            const venta = SalesService.getById(id);
            let sale = SalesService.deleteById(id);
            if(!sale) {
                Response.error(res, new createError.NotFound());
            } else {
                Response.success(res, 200, `Venta ${id} eliminada`, venta);
            }
        } catch (error) {
            debug(error);
            Response.error(res);
